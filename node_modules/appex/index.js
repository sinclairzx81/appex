    ...
    (function (servicehost) {
        var Service = (function () {
            ...
            Service.prototype.initialize = function () {
                if (!this.instance) {
                    this.instance = this.handle.activate();
                }
            };

            Service.prototype.call = function (method, input) {
                this.initialize();
    ...
    (function (servicehost) {
        var Route = (function () {
            ...
            Route.prototype.call = function (input, callback) {
                return this.method(input, callback);
            };
            return Route;
        })();
...
(function (appex) {
    (function (servicehost) {
        var Host = (function () {
            ...
            Host.prototype.setup_services = function () {
                var _this = this;
                this.services = new Array();

                this.activation.handles.forEach(function (handle) {
                    var service = new appex.servicehost.Service(handle);
            ...
            Host.prototype.setup_routes = function () {
                ...
                this.services.forEach(function (service) {
                    var basepath = '/' + service.handle.typename.replace(/\./g, '/');

                    service.handle.typeinfo.methods.forEach(function (method) {
                        if (method.name != 'constructor') {
                            var route = new appex.servicehost.Route();

                            route.path = [basepath, method.name].join('/').toLowerCase();

                            route.method = service.instance[method.name];
...
exports.create_activation_context = create_activation_context;
...
exports.create_service_host = create_service_host;

function create(source, callback) {
    exports.create_runtime(source, function (runtime) {
        exports.create_activation_context(runtime, function (activation_context) {
            exports.create_service_host(activation_context, function (service_host) {
                callback(service_host);
            });
        });
    });
    ...
    exports.create(source, function (host) {
        ...
        server.on('request', function (request, response) {
            for (var n in host.routes) {
                if (request.url == host.routes[n].path) {
                    var buffer = [];
                    ...
                    request.on('end', function () {
                        ...
                        try  {
                            if (!host.routes[n].async) {
                                console.log('synchronous');

                                var input = JSON.parse(json);

                                var output = host.routes[n].call(input);
                            ...
                            if (host.routes[n].async) {
                                ...
                                host.routes[n].call(input, function (output) {
                                    var json = JSON.stringify(output, null, ' ');
