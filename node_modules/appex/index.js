    ...
    var StopWatch = (function () {
        ...
        StopWatch.prototype.stop = function () {
            if (this.starttime) {
                ...
                return stoptime.getMilliseconds() - this.starttime.getMilliseconds();
            }
    ...
    var Route = (function () {
        ...
        Route.prototype.setup_url = function (method) {
            ...
            if (this.moduleExport.scope.length == 0) {
                ...
                this.url = name == 'index' ? ('/' + this.moduleExport.scope.join('/')) : ('/' + this.moduleExport.scope.join('/') + '/' + name);
            }
        };
            ...
            typescript.resolve([filename], function (resolved) {
                if (!typescript.check(resolved)) {
                    callback({
                        script: '',
                        reflection: null,
                        errors: gather_diagnostics(resolved)
                    });
                ...
                typescript.compile(resolved, function (compiled) {
                    if (!typescript.check(compiled)) {
                        callback({
                            script: '',
                            reflection: null,
                            errors: gather_diagnostics(compiled)
                        });
        ...
        Runtime.prototype.handler = function (request, response, next) {
            ...
            for (var n in this.routes) {
                ...
                if (url.pathname == route.url) {
                    ...
                    var appex_request = request;

                    var appex_response = response;
...
function handle_errors(diagnostics, response) {
...
function runtime(options) {
    var prepare_options = function (options) {
